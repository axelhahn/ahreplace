 ...
 * REPLACE TAGS
 * 
 * Class for text freplacement by using text sources for pre defined tags.
 * 
...
class ReplaceTags {
     ...
     * @return boolean
     */
    function __construct($sTagfile = false) {
        if ($sTagfile) {
     ...
     */
    private function _tag_value_replace($sFrom, $sTo, $line) {
        $oldval = $sTo;
     ...
     * @return boolean
     */
	public function addTag($sTag, $sReplace){
        $this->_aTags[]=[
            "tag"     => trim($sTag),
     ...
     * @return boolean
     */
    public function readTagfile($sTagfile = false) {
		$sTag='';
		$sReplace='';
     ...
     * @param  string   $iRuns     optional: repeats; default: 1 run
     * @return string the processed line of text
     */
    public function doReplace($line, $iRuns=1) {
